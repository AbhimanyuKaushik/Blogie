...
function extractPreview(doc?: DocumentAST, max = 220) {
  ...
  for (const t of preferred) {
    ...
    if (b) {
      ...
      if (txt) return txt.slice(0, max) + (txt.length > max ? "â€¦" : "");
    }
  }

  const all = doc.blocks.map((b) => getTextFromInline(b.content)).join("\n\n");
  return all.slice(0, max) + (all.length > max ? "â€¦" : "");
}
...
export default function PostCard({
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return; // 400 errors (already liked/not liked) are expected â†’ silent revert
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/posts/${post._id}/save`,
        {
